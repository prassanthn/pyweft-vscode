...
const toVs = (p) => new vscode.Position(p.line, p.character);
const fromVs = (p) => ({ line: p.line, character: p.character });
function mapRange(entry, r) {
    const s = entry.doc.toWeft(fromVs(r.start));
    const e = entry.doc.toWeft(fromVs(r.end));
    if (s && e)
        return new vscode.Range(toVs(s), toVs(e));
...
async function locate(docs, document, position) {
 ...
 */
function throughAlias(entry, vpos) {
    ...
    while ((m = re.exec(line))) {
        ...
        if (m.index > vpos.character)
            break;
    }
    return vpos;
}
function registerProviders(context, docs) {
    const selector = { language: "weft" };
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider(selector, {
        async provideCompletionItems(document, position, _token, ctx) {
            const at = await locate(docs, document, position);
            if (!at)
                return undefined;
            const list = await vscode.commands.executeCommand("vscode.executeCompletionItemProvider", at.entry.uri, at.vpos, ctx.triggerCharacter, 30);
            if (!list)
                return undefined;
        ...
        async provideHover(document, position) {
            ...
            const hovers = await vscode.commands.executeCommand("vscode.executeHoverProvider", at.entry.uri, target);
            const h = hovers?.[0];
            if (!h)
                return undefined;
        ...
        async provideDefinition(document, position) {
            ...
            if (!at)
                return undefined;
            const found = await vscode.commands.executeCommand("vscode.executeDefinitionProvider", at.entry.uri, throughAlias(at.entry, at.vpos));
            const out = [];
            for (const loc of found ?? []) {
                ...
                if (uri.toString() === at.entry.uri.toString()) {
                    ...
                    if (mapped)
                        out.push(new vscode.Location(document.uri, mapped));
                }
                else {
        ...
        async provideSignatureHelp(document, position, _token, ctx) {
            ...
            if (!at)
                return undefined;
            return vscode.commands.executeCommand("vscode.executeSignatureHelpProvider", at.entry.uri, at.vpos, ctx.triggerCharacter);
        },
    }, "(", ","));
}
