...
const TAG_RE = /<!--[\s\S]*?-->|<(\/?)([A-Za-z_][\w.\-:]*)((?:\s+[^\s=>\/]+(?:\s*=\s*(?:"[^"]*"|'[^']*'))?)*)\s*(\/?)>/g;
...
class Builder {
    ...
    emitMapped(indent, prefix, text, start, suffix, kind) {
...
function collectNames(line, minIndent, into) {
    const indent = /^[ \t]*/.exec(line)[0].length;
    if (indent !== minIndent)
        return;
    const s = line.trim();
    let m;
    if ((m = /^(?:async\s+)?def\s+([A-Za-z_]\w*)/.exec(s)))
        into.add(m[1]);
    else if ((m = /^class\s+([A-Za-z_]\w*)/.exec(s)))
        into.add(m[1]);
    else if ((m = /^import\s+([\w.]+)(?:\s+as\s+(\w+))?/.exec(s)))
        into.add(m[2] ?? m[1].split(".")[0]);
 ...
 */
function topLevelBlocks(codeLines, minIndent) {
    const blocks = [];
    let depth = 0;
    let afterDecorator = false;
    const strip = (l) => l.replace(/("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g, "").replace(/#.*$/, "");
...
function walkTemplate(content, contentOffset, b, idx) {
    let indent = 8;
    const stack = [];
    const closeFor = (bodyStart) => {
        if (b.lines.length === bodyStart)
            b.emit(" ".repeat(indent) + "pass");
        indent -= 4;
    };
    const interpolations = (text, textOffset) => {
    ...
    for (const m of content.matchAll(TAG_RE)) {
        ...
        for (const a of attrs) {
            if (isFor && (a.name === "each" || a.name === "index"))
                continue;
    ...
    if (code) {
        ...
        codeLines.forEach((line, i) => {
            if (line.trim()) {
                ...
                if (mod !== undefined)
                    moduleDup.set(mod, { line: b.lines.length, shift: 4 + minIndent });
